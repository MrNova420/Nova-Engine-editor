 ...
 */
export interface User { id: string; email: string; username: string; createdAt: Date; roles: string[]; }
export interface AuthToken { accessToken: string; refreshToken: string; expiresIn: number; }
export class AuthService {
  ...
  async register(email: string, username: string, password: string): Promise<User> {
    ...
    const user: User = { id: userId, email, username, createdAt: new Date(), roles: ['user'] };
